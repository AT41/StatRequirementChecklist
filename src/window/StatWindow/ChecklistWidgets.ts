...
export class ChecklistWidgets {
    ...
    private static buildWidgets(xStart: number, yStart: number): WidgetDesc[] {
        ...
        this.Y_START = yStart;
        let createChecklistRequirements = (
            xStart: number,
            yStart: number,
            withInversion?: InversionRequired,
        ): WidgetDesc[] => {
        ...
        let createChecklistPart = (
            ...
            return checklistPart.concat(
                createChecklistRequirements(
                    xStart,
                    yStart + this.LABEL_HEIGHT,
                    withInversion,
                ),
            );
        };
        ...
        widgets = widgets.concat(
            createChecklistPart(
                InversionRequired.WithInversion,
                xStart,
                yStart,
            ),
            createChecklistPart(
                InversionRequired.WithoutInversion,
                xStart,
                yStart,
            ),
    ...
    public static updateWidgets(
        rideRequirements: RideRequirement | undefined,
        yStart?: number,
    ): number {
        const toWidgetNames = (
            rideRequirements: RideRequirement | undefined,
            ...
            if (rideRequirements?.optional_inversion === true) {
                Object.keys(rideRequirements ?? {}).forEach((key) => {
                    ...
                    const measurement = (
                        (rideRequirements ?? ({} as any))[keyOfRideReq] ?? ''
                    ).toString();
                    const inversionRequired =
                        measurement.indexOf('*') !== -1
                            ? InversionRequired.WithInversion
                            : InversionRequired.WithoutInversion;
                    widgetNames[
                        this.getWidgetName(
                            key as keyof RideRequirement,
                            inversionRequired,
                        )
                    ] = measurement;
                });
                widgetNames[
                    this.getWidgetName('', InversionRequired.WithInversion)
                ] = 'With Inversion';
                widgetNames[
                    this.getWidgetName('', InversionRequired.WithoutInversion)
                ] = 'Without Inversion';
            } else {
                Object.keys(rideRequirements ?? {}).forEach((key) => {
                    const keyOfRideReq = key as keyof RideRequirement;
                    widgetNames[
                        this.getWidgetName(key as keyof RideRequirement)
                    ] = Logic.formatString(
                        (
                            (rideRequirements ?? ({} as any))[keyOfRideReq] ??
                            ''
                        ).toString(),
                        RideRequirementValueFormatMap[keyOfRideReq],
                    );
                });
            }
            return widgetNames;
        ...
        for (let widget of this.widgets) {
            ...
            if (widgetNames[widget.name ?? '']) {
                ...
                (windowWidget as LabelWidget).text = (
                    windowWidget as LabelWidget
                ).text.replace(/: *$/, ': ' + widgetNames[widget.name ?? '']);
    ...
    private static getWidgetName(
        widgetType: keyof RideRequirement | '',
        inversion?: InversionRequired,
    ): string {
        let inversionPart;
