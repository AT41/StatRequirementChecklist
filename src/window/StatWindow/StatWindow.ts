...
export class StatWindow {
    ...
    public static createWindow(): Window {
        ...
        this.window = ui.openWindow({
            ...
            height: 300,
            x: rideEditWindow
                ? rideEditWindow.x + rideEditWindow.width
                : undefined,
    ...
    public static setHooks() {
        ...
        this.hooks.push(
            context.subscribe('action.execute', (e) => {
                ...
                let createOrTrackPlace: 'ridecreate' | 'trackplace' =
                    'trackplace';
                switch (e.action) {
                    //@ts-ignore
                    case 'ridecreate':
                    ...
                    case 'trackplace':
                        ...
                        let rideId: number | undefined = (
                            e.args as TrackPlaceArgs
                        ).ride;
                        if (rideId === undefined)
                            rideId = (e.result as RideCreateActionResult).ride;
                        let rideType: number = (
                            e.args as TrackPlaceArgs | RideCreateArgs
                        ).rideType;
                        const currentRideId =
                            RideSelectWidgets.getCurrentRideId();
                        ...
                        if (
                            ...
                            const rideSelectWidget = this.window.findWidget(
                                'rideSelectWidgetId',
                            ) as DropdownWidget;
                            if (!rideSelectWidget) break;
                            let rideDropdownIndex = rideSelectWidget.items
                                .map(
                                    (idAndName) => idAndName.match(/^\d+/)?.[0],
                                )
                                .indexOf((rideId ?? 0).toString());
                            rideDropdownIndex =
                                rideDropdownIndex === -1
                                    ? 0
                                    : rideDropdownIndex;
                            RideSelectWidgets.updateRideSelectDropdown(
                                rideDropdownIndex,
                            );
                        }
                        break;
        ...
        this.hooks.push(
            context.subscribe('ride.ratings.calculate', (e) => {
                if (
                    OptionWindow.data.options.autoUpdateChecklistSelection &&
                    e.rideId === RideSelectWidgets.getCurrentRideId() &&
                    getPluginWindow()
                ) {
                    let requirements = Logic.getRequirements(
                        map.getRide(e.rideId).type,
                    );
                    if (!requirements) return;
                    const checklistYEnd =
                        ChecklistWidgets.updateWidgets(requirements);
