import { RIDE_REQUIREMENT_OBJ, RideRequirement, RideRequirementLabelMap, RideRequirementValueFormatMap } from "../../data/types";
...
export class RideRequirementWidgets {
    ...
    public static updateWidgets(window: Window, statRequirements: RideRequirement | null): number {
        ...
        Object.keys(rideReqs).forEach((key) => {
            ...
            widget.text = widget.text.replace(/: .*$/, ": " + Logic.formatString(statRequirements?.[keyAsReq] ? statRequirements[keyAsReq] as string : "", RideRequirementValueFormatMap[keyAsReq]));
        ...
        let groupboxYEnd = this.Y_START + (numberOfLines + 1) * (this.Y_INCREMENT_HEIGHT) + 5;
    ...
    private static buildWidgets(startX: number, startY: number): WidgetDesc[] {
        ...
        descriptionWidgets = descriptionWidgets.concat(Object.keys(RIDE_REQUIREMENT_OBJ).filter(key => key !== "ride_type").map(
            (key => {
